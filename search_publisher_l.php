...
require "var.inc.php";
if(isset($_POST['searchBook_P']))
{
	$input = $_POST['searchBook_P'];
}
?>
		...
		<div id="login_pg">
   			...
   			<br>					
					<?php
							...
							*/
							$query = "SELECT DISTINCT `book`.`availability` as `avaibility`, `book`.`book_title` as `title`, `book`.`category` as `category`,`book`.`book_id` as `book_id`  FROM  `book`,  `publisher`, `author` WHERE  `publisher`.`publisher_name` LIKE '%$input%' AND `book`.`book_id`=`publisher`.`book_id`";
							$query_run = mysqli_query($mysql_connect , $query);
